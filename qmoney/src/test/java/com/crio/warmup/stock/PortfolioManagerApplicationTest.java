...
class PortfolioManagerApplicationTest {
  ...
  void mainCalculateAnnualReturn() throws IOException, URISyntaxException {
    ...
    // then
    List<String> symbols = result.stream().map(
        AnnualizedReturn::getSymbol).collect(Collectors.toList());
  ...
  @Test
  public void testCalculateAnnualizedReturnGoogl() throws IOException, URISyntaxException {
